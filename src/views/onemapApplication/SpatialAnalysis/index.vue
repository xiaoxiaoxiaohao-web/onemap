<template>
  <RightPanel :show="rightPanelShow" ref="spatialAnalysisRightPanel">
    <div :class="isShowResult?'spatial-analysis-r':'spatial-analysis-q'">
      <Query v-show="!isShowResult" @changeIsShowResult="changeIsShowResult" @changLoading="changLoading" @returnResult="returnResult"></Query>
      <Result ref="SAResult" v-show="isShowResult" @changeIsShowResult="returnQuery" :data="tableDataList" :isShowContorl="isShowContorl"></Result>
    </div>
  ...
  methods: {
    ...
    changeIsShowResult(res) {
      ...
      for (let i in res) {
        // 表头数据
        let tableHeaderList = []
        for (let n in res[i].resultJsonObj.fieldAliases) {
          let obj = {
            name: res[i].resultJsonObj.fieldAliases[n],
            value: 'attributes.' + n
          }
          tableHeaderList.push(obj)
        }
        // 全部内容
        let tableObj = {
          heard: res[i].module.moduleResultName.replace('分析结果', ''),
          vueId: res[i].module.moduleId, // id
          title: res[i].module.moduleResultName,
          layerUrl: res[i].module.layer.layerUrl,
          conclusion: '', // 结论
          tableObj: {
            tableHeaderList: tableHeaderList, // 表头
            tableDataList: res[i].resultJsonObj.features // 数据
          },
          fieldAliases: res[i].resultJsonObj.fieldAliases
        }
        this.tableDataList.push(tableObj)
      }
      this.isShowResult = true
      this.$refs.SAResult.handleData(this.tableDataList)
    },
    // 返回查询
    returnQuery() {
