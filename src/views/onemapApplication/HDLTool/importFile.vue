 ...
 * @Description: 项目红线  导入文件
 * @Author: ljl
 * @Date: 2019-08-27 15:40:09
 -->
<template>
  <div class="import-file" cid="importFile">
    ...
    <div class="bottom-div">
      <el-button type="danger" @click="closeWin">取消</el-button>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    beforeUpload(file) {
      ...
      HDLTool.importFile(param).then(response => {
        console.log('导入')
    ...
    successFun(response, file, fileList) {
      ...
      let getGeoList = response.json // string类型
      this.$message({
        type: 'success',
        message: '上传成功'
      })
      this.$emit('closeWinFun', false)
