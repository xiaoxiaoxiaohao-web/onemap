 ...
 *     b)rightPanelShowKey state中存放右侧面板是否显示的key
 *     c)rightPanelWidthKey state中存放右侧面板的宽度的key
 *     d)setRightPanelShowKey mutation中设置右侧面板是否显示的方法key
 *     e)setRightPanelWidthKey mutation中设置右侧面板的宽度的方法key
 *   3、在store中该组件存放数据的模块中，在state中添加rightPanelShowKey,rightPanelWidthKey,mutaion中添加setRightPanelShowKey,setRightPanelWidthKey为对应的设置前两个变量的方法
 *   4、在路由展示的根组件中，需要自己写获取rightPanel宽度的方法
 *   5、在路由展示的根组件中，需要自己监听rightPanel插槽中组件变化，在方法中调用this.setRightPanelWidth(自定义的获取rightPanel宽度的方法的返回值)。注意一定要包裹在nextTick中
...
export default {
  ...
  methods: {
    // 设置右侧面板是否显示
    setRightPanelShow(show) {
      this.$store.commit(`${this.rightPanelNameSpace}/${this.setRightPanelShowKey}`, show)
    },
  ...
  computed: {
    rightPanelShow() {
      return this.$store.state[this.rightPanelNameSpace][this.rightPanelShowKey]
    },
    rightPanelWidth() {
  ...
  watch: {
    // 当show改变的时候，提交store
    rightPanelShow() {
      this.$store.commit(this.rightPanelNameSpace + '/setRightPanelShow', this.rightPanelShow)
    },
    // 当右侧面板宽度改变的时候，提交store
    rightPanelWidth() {
      this.$store.commit(this.rightPanelNameSpace + '/setRightPanelWidth', this.rightPanelWidth)
    }
  },
